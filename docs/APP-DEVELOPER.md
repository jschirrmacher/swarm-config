...
1. **Create repository** via Web UI at `https://config.$DOMAIN`
   - Enter name, set port
...
Your app is live at `https://myapp.$DOMAIN`.
...
**Why?** The deployment runs from `$WORKSPACE_BASE/<owner>/<project>/` (default: `/var/apps/<owner>/<project>/`), and relative paths work correctly from there. This also makes your compose.yaml portable for local development.
...
1. **Create/edit `.env` on the server** at `$WORKSPACE_BASE/<owner>/<appname>/.env`:

```bash
ssh example.com
nano $WORKSPACE_BASE/<owner>/<appname>/.env
```
...
```bash
# Live logs
ssh $DOMAIN 'docker service logs -f myapp_myapp'

# Service status
ssh $DOMAIN 'docker service ps myapp_myapp'

# Check environment variables
ssh $DOMAIN 'docker exec $(docker ps --format "{{.Names}}" | grep myapp) env'

# Shell into container
ssh $DOMAIN 'docker exec -it $(docker ps -q -f name=myapp_myapp) sh'
```
...
```bash
ssh $DOMAIN 'tail -f $WORKSPACE_BASE/<owner>/<app>/post-deploy.log'
```
...
- Check logs: `tail -50 $WORKSPACE_BASE/<owner>/<app>/post-deploy.log`
...
The server automatically manages file permissions for your projects. All files use a shared group model where:

- Your workspace at `$WORKSPACE_BASE/<owner>/<project>/` is readable and writable by you and the deployment system
- New files automatically get the correct permissions (docker group with write access)
- The git hook and Docker containers can modify files in your project directory

**No action needed from developers.** The system ensures git push, container operations, and manual file editing all work seamlessly together.
...
| Not reachable          | DNS (`nslookup myapp.example.com`), firewall (`sudo ufw status`) |
