  ...
  for (const service of config.services) {
    ...
    for (const route of service.routes) {
      ...
      if (route.options && Object.keys(route.options).length > 0) {
        ...
        if (route.options.strip_path !== undefined)
          opts.push(`strip_path: ${route.options.strip_path}`)
        if (route.options.preserve_host !== undefined)
          opts.push(`preserve_host: ${route.options.preserve_host}`)
